...
**Two local folders, one real remote — easy to mix up.** `astryks-app` (this folder) is a local-only git repo with no configured remote — commits here are just local history, they never reach GitHub or trigger a deploy on their own. `Astryks-GitHub` (a sibling folder, remote: `https://github.com/Astryks/Astryks.git`, web app under its `web/` subfolder) is the repo Firebase App Hosting actually watches and deploys from. Any change that needs to go live must be mirrored into `Astryks-GitHub` and pushed from there — pushing from `astryks-app` alone does nothing.
